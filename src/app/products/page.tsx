...
function ProductsContent() {
  ...
  const [topRevenue, setTopRevenue] = useState<any>(null)
  const [topQuantity, setTopQuantity] = useState<any>(null)
  ...
  useEffect(() => {
    ...
    fetch(`/api/products/top?range=${encodeURIComponent(range)}&by=quantity`)
      .then(r => r.json())
      .then(data => setTopQuantity(data))
  }, [range])

  useEffect(() => {
    fetch(`/api/products?range=${encodeURIComponent(range)}&q=${encodeURIComponent(debouncedSearch)}`)
      .then(r => r.json())
